...
syntax [if] [in] [,   Y(string) D(string) Z(string) first(real 0)  median  periods(real 1) cohort_treatment_date(string) time(string) exponential controls(varlist) keep(real 1)  ]        
/*         PARSE TEXT       */
marksample _sample
// 	set buildfvinfo on
tempvar copy 
...
qui: bys `_sample' `cohort_treatment_date' `time'  : gen `total_dates' = (_n==`keep') if `_sample' // & (`cohort_treatment_date' > 0) // at least ten observations per cohort and date 
 qui: bys `_sample' `cohort_treatment_date'  : egen `sum_dates' = sum(`total_dates')   if `_sample' //& (`cohort_treatment_date' > 0) // check if condition validates across all dates 
 qui: bys `_sample' `cohort_treatment_date' `time'  : gen `nb_dates' = (_n==1) if `_sample' //& (`cohort_treatment_date' > 0) // count number of dates
 qui: bys `_sample' `cohort_treatment_date'  : egen `sum_nb_dates' = sum(`nb_dates')   if `_sample' //& (`cohort_treatment_date' > 0) // check if condition validates across all dates 
 qui : replace `_sample' = 0  if (`sum_dates'< `sum_nb_dates')  & `_sample' //& (`cohort_treatment_date' > 0) // keep only valid cohorts 
 tempvar maxdate 
 qui: egen `maxdate' = max(`cohort_treatment_date')
 qui : replace `_sample' = 0 if `cohort_treatment_date'==0 &  ((`time' > (`maxdate' + `periods'))) & `_sample'
tempvar lnY
